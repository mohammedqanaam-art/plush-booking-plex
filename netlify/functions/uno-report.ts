...
// Netlify Background Functions may run for 15 minutes. Keep a longer lease so
// a second scheduled run cannot overlap a legitimate monthly UNO export.
...
const executeReport = async (
  ...
  if (!active) {
    await updateSyncHealth(false, filters, {
      source,
      error: "انتهت جلسة UNO وتحتاج OTP.",
      requiresOtp: true,
  ...
  try {
    ...
    if (canonicalUpdated) {
      ...
      await updateSyncHealth(true, filters, {
        source,
        total: fetched.reservations.length,
        durationMs: Date.now() - startedAt,
      }).catch(() => undefined);
