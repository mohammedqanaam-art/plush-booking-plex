...
export function HotelInformationContent() {
  ...
  return <div className="page-wrap" dir="rtl">
    <PageHeader title="معلومات الفنادق" icon={Building2} actions={<Link to="/branches" className="text-sm text-primary underline">دليل الفروع</Link>} />
    ...
    {!selected ? <p role="status" className="page-surface text-center">{id ? "الفرع المطلوب غير موجود ضمن البراند المحدد. اختر فرعًا من القائمة." : "اختر الفرع لعرض جميع تفاصيله المسجلة."}</p> : <>
      ...
      {groups.map(group => <section key={group.title} className="page-surface space-y-4"><h3 className="text-lg font-bold text-primary">{group.title}</h3><dl className="grid gap-4 sm:grid-cols-2">{group.fields.map(([field, label]) => <div key={field} className="min-w-0 rounded-xl border bg-secondary/20 p-4"><dt className="mb-2 text-sm text-muted-foreground">{label}</dt><dd className="whitespace-pre-wrap break-words text-base leading-7" dir="auto">{text(selected[field])}</dd></div>)}</dl></section>)}
      <section className="page-surface space-y-4"><h3 className="text-lg font-bold text-primary">الغرف والأجنحة</h3>
        ...
        <p className="text-sm text-muted-foreground">{selected.roomSource === "unverified" ? "قائمة الغرف غير متحققة؛ يلزم تأكيدها مع الفرع قبل إفادة الضيف." : "المعلومات من المصدر الداخلي؛ تُراجع الإتاحة والأسعار في نظام الحجز."}</p>
      </section>
      ...
      <section className="page-surface space-y-4"><h3 className="text-lg font-bold text-primary">الملاحظات والمصادر</h3><p className="whitespace-pre-wrap leading-7">{text(selected.notes)}</p>{extra?.notes && extra.notes !== selected.notes && <p className="whitespace-pre-wrap leading-7">{extra.notes}</p>}
